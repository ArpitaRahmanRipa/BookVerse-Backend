...
export default function RecommendationPage() {
  ...
  const loadHistory = async () => {
    try {
      ...
      const result =
        await getUserRecommendations(USER_ID);
  ...
  const handleGenerate = async () => {
    try {
      ...
      const favoriteGenres = form.favoriteGenres
        .split(",")
      ...
      setMessage({
        ...
        text:
          result.data.source === "openai"
            ...
            : "Recommendations generated using your reading context (OpenAI key not configured).",
      });
    } catch (error) {
      setMessage({
        type: "error",
      ...
      <main className="mx-auto max-w-6xl px-6 py-16">
        <div className="rounded-3xl bg-white p-10 text-center shadow-sm">
          <p className="text-lg text-stone-600">
            Loading recommendation assistant...
          </p>
        </div>
      </main>
    );
    ...
    <main className="mx-auto max-w-6xl px-6 py-10">
      <section className="mb-8">
        ...
        <p className="mt-2 max-w-3xl text-stone-600">
          Get personalized book suggestions based on your
          reading history, favorite genres, mood, and
            ...
            <label>
              ...
              >
                {moodOptions.map((mood) => (
                  <option key={mood || "any"} value={mood}>
                    {mood || "Any mood"}
                  </option>
                ))}
              </select>
        ...
        <section className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
          ...
          {!latestResult?.recommendations?.length ? (
            <p className="mt-5 text-stone-500">
              No recommendations yet. Submit a request to
              get started.
            </p>
          ) : (
      ...
      <section className="mt-6 rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
        ...
        {history.length === 0 ? (
          <p className="mt-4 text-stone-500">
            Your past recommendation requests will appear
            here.
          </p>
        ) : (
          <div className="mt-6 space-y-4">
